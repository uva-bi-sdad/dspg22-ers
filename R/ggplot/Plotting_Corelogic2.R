...
for (state_id in state_fips){
  q = paste0("SELECT fips_code, parcel_level_latitude, parcel_level_longitude, assessed_total_value
             FROM
             corelogic_usda.combined_propertybasic_limited
             WHERE  property_indicator_code = '10'
             AND parcel_level_latitude IS NOT NULL 
             AND parcel_level_latitude != ''
             AND parcel_level_longitude IS NOT NULL 
             AND parcel_level_longitude != ''
...
dbDisconnect(con)
datalist$parcel_level_longitude <- as.numeric(datalist$parcel_level_longitude)
datalist$parcel_level_latitude <- as.numeric(datalist$parcel_level_latitude)
...
ggplot() + geom_sf(data = states$geometry[states$GEOID %in% state_fips$state_fips], fill = state_fips$state_fips) +
  ...
  geom_point(data=CL_outside, aes(x=parcel_level_longitude, y=parcel_level_latitude), color="orange", size=0.5) +
  geom_point(data=CL_inside, aes(x=parcel_level_longitude, y=parcel_level_latitude), color="lightgreen", size=0.5) 
